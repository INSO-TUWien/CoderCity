...
describe('Kd-treenode', () => {
    it('should insert elements correctly', () => {
        ...
        const bounds = new Bounds(5, 5);

        const element = new Element(new Bounds(3, 3));
        const node = new KDTreeNode(position, bounds, 0, 2);
        const result = node.insertElement(element);
    ...
    it('should have valid position in splitted child nodes after inserting Element of size 3x3 and 1x1', () => {
        ...
        const element = new Element(new Bounds(3, 3));
        const element2 = new Element(new Bounds(1, 1));

        const node = new KDTreeNode(position, bounds, 0, 2);
        const result = node.insertElement(element);
        expect(result).toBe(true);

        const result2 = node.insertElement(element2);
